...
namespace Navmii.AirtableSync
{
    public class TSheetsFetch
    {
        ...
        public Dictionary<int, Dictionary<string, string>> FetchUsers()
        {
            ...
            HashSet<long> customFieldIDs = new HashSet<long>();
            foreach (User user in listUsers)
            {
                if (user.GroupId.HasValue && !groupIDs.Contains((long)user.GroupId.Value))
                {
                    groupIDs.Add((int)user.GroupId.Value);
                }
            ...
            foreach (User user in listUsers)
            {
                ...
                if (user.GroupId.HasValue && groups.TryGetValue((int)user.GroupId.Value, out Group group))
                {
